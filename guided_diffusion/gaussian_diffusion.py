...
class GaussianDiffusion:
        ...
        if progress:
           ...
           for i in indices:
                ...
                if i%100==0:
                    ...
                    viz.image(visualize(img.cpu()), opts=dict(caption="sample" + str(i)))
                    viz.image(visualize(img.cpu()[0, -1,...]), opts=dict(caption="sample"+ str(i) ))

                with th.no_grad():
                    if img.shape != (1, 2, 224, 224):
                        img = torch.cat((org_MRI,img), dim=1)       #in every step, make sure to concatenate the original image to the sampled

                    #print('i p sample loop progr', str(i))
                    out = self.p_sample(
                        model,
                        img.float(),
                        t,
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        res_t = self.q_sample(goal, t, noise=noise)     #add noise to the BL channel, FU stays "clean"
        x_t=x_start.float()
        x_t[:, -1:, ...]=res_t.float() # replace last channel by noisy BL
        terms = {}
        #viz.image(x_t[0,0,...], opts=dict(caption='clean FU'))
        #if t < 200:
        #    viz.image(visualize(x_t[0, 1, ...]), opts=dict(caption='noisy BL'))
        #viz.image(goal[0, 0, ...], opts=dict(caption='clean BL'))
